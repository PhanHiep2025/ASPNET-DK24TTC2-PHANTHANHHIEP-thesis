...
namespace ShoesStore.Controllers
{
    public class PhieuMuaController : Controller
    {
        ...
        public IActionResult ThanhToan(PhieuMuaViewModel phieuMua)
        {
            ...
            if (Check(phieuMua) == true)
            {
                ...
                if (phieuMua.maTinh != 0)
                {
                    ...
                    List<Phuong> phuongList = addressRepo.GetPhuongList(phieuMua.maTinh);
            ...
            string receiver = "phanhiepcc2026@gmail.com";
            string subject = "Đơn hàng mới:";
            string message = "Mã đơn hàng: ";
        ...
        public IActionResult AddressNoteBook()
        {
            ...
            Khachhang kh = khRepo.GetCurrentKh("phanhiepcc2026@gmail.com");
            pm.khInfo = kh;
        ...
        public IActionResult AddAddress(int province, int district, string address, int makh, string tennguoinhan, string sdtnguoinhan)
        {
            addressRepo.AddAddressNote(province, district, address, makh, tennguoinhan, sdtnguoinhan);
            return RedirectToAction("AddressNoteBook");
        }

        public IActionResult GetDistinctsByProvince(int provinceId)
        {
            List<Phuong> phuongList = addressRepo.GetPhuongList(provinceId);
            List<SelectListItem> selectPhuong = phuongList.Select(x => new SelectListItem
            {
                Value = x.Maphuong.ToString(),
                Text = x.Tenphuong
        ...
        public IActionResult UpdateAddress(int masdc)
        {
            ...
            int Maphuong= addressRepo.GetMaPhuong(sdc.Diachi.Split(',')[2].TrimStart());
            ...
            List<Phuong> phuongList = addressRepo.GetPhuongList(maTinh);
            List<SelectListItem> selectPhuong = phuongList.Select(x => new SelectListItem
            {
                Value = x.Maphuong.ToString(),
                Text = x.Tenphuong
        ...
        public IActionResult UpdateAddressFinal(int masdc, string hoten, string sdt, string diachi, int matinh, int maphuong)
        {
            addressRepo.UpdateSDC(masdc, hoten, sdt, diachi, matinh, maphuong);
        ...
        public bool Check(PhieuMuaViewModel phieuMua)
        {
            ...
            if (phieuMua.maTinh == 0)
            {
                ModelState.AddModelError("maTinh", "Chọn tỉnh");
                check = true;
            }
            if (phieuMua.Maphuong== 0)
            {
                ModelState.AddModelError("maPhuong", "Chọn phương/xã");
                check = true;
            }
        ...
        public IActionResult ChangeAddressPhieuMua(int masodiachi)
        {
            ...
            string emailkh = HttpContext.Session.GetString("Email") ?? "phanhiepcc2026@gmail.com";
            Khachhang kh = khRepo.GetCurrentKh(emailkh);
            ...
            PhieuMuaViewModel pmview = new PhieuMuaViewModel()
            {
                ...
                Maphuong= addressRepo.GetMaPhuong(sdc.Diachi.Split(',')[2].TrimStart()),
               

            };

            List<Phuong> phuongList = addressRepo.GetPhuongList(pmview.maTinh);
            List<SelectListItem> selectPhuong = phuongList.Select(x => new SelectListItem
            {
                Value = x.Maphuong.ToString(),
                Text = x.Tenphuong
